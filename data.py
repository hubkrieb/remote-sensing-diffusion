...
class SegmentationDataset(data.Dataset):
        def __init__(self, db_path, split, n_channels = 3):
            ...
            with open(f'dataset/{split}.csv') as f:
                reader = csv.reader(f)
...
def get_dataloader(db_path, n_channels, batch_size, split, num_workers = 0):
    ...
    dataset = SegmentationDataset(db_path, split, n_channels)
    dataloader = data.DataLoader(dataset = dataset, batch_size = batch_size, shuffle = True, num_workers = num_workers)
    return dataloader